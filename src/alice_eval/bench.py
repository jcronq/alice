...
Provides the four ``alice_eval speaking <sub>`` subcommands:

- ``sample``    — re-exports :func:`alice_eval.sampling.main`
  for the SWE-Bench naming
- ``instances`` — derive per-turn assertion files from the sample
  (see :mod:`alice_eval.instances`)
...
The runner reuses :mod:`alice_eval.replay`'s candidate calling. We
load the same ``configs/eval_candidates.json``, filter to one
candidate by ``--candidate <id>``, and invoke
:func:`alice_eval.replay.replay_turn` per instance.
"""
...
from alice_eval import assertions as _assertions
from alice_eval import instances as _instances
from alice_eval import sampling as _sampling
from alice_eval import score as _score
from alice_eval.prompt import build_system_prompt
from alice_eval.replay import (
    DEFAULT_CONCURRENCY,
    DEFAULT_TIMEOUT_S,
    Candidate,
    load_candidates,
...
# ---------------------------------------------------------------------------
# CLI plumbing for `alice_eval speaking <sub>`


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m alice_eval speaking",
    ...
    sample_p = sub.add_parser(
        "sample",
        help="(alias for `python -m alice_eval sample`)",
    )
