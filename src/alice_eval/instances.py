...
Reads ``eval_sample.jsonl`` (produced by :mod:`alice_eval.sampling`),
...
from alice_eval.assertions import AssertionFile, extract_tool_names
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="alice_eval.instances",
        description="Derive per-instance assertion files from a sample.",
    )
    p.add_argument(
        "--sample",
        default="eval_sample.jsonl",
        help="Path to the eval_sample.jsonl produced by `alice_eval sample`",
    )
    p.add_argument(
        "--out-dir",
