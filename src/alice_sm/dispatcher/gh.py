...
kwargs on :func:`alice_sm.dispatcher.run`).
"""
...
def _dispatch_run_gh(args: list[str], **kwargs: Any) -> str:
    ...
    Tests monkeypatch ``alice_sm.dispatcher._run_gh`` directly (see
    e.g. ``test_gh_get_pr_files_parses_payload``). Pre-split, every
    ``gh_*`` helper resolved ``_run_gh`` via a global lookup in
    ``alice_sm.dispatcher``'s own module dict, so the monkeypatch took
    ...
    """
    return sys.modules["alice_sm.dispatcher"]._run_gh(args, **kwargs)
