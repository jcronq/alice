"""The seven Stage B workflow steps.

Each step is a typed function. Steps that need LLM judgement take a
:class:`ModelCall` callable as the model interface — production wires
:func:`make_default_model_call`; tests inject a fake.

Per-step contracts mirror the design sketch
(``docs/designs/stage-b-adk-workflow-sketch.md``):

1. :func:`read_wake_state` — deterministic. Filesystem reads only.
2. :func:`drain_inbox` — LLM per note, deterministic apply.
3. :func:`pick_grooming_target` — deterministic scoring.
4. :func:`groom_target` — LLM produces typed Diff, deterministic apply.
5. :func:`side_checks` — three parallel branches via asyncio.gather.
6. :func:`emit_surfaces` — deterministic.
7. :func:`close` — deterministic; writes wake summary, runs prune.

Steps update :class:`WakeState` in-place where needed (so later steps
can read what earlier steps did) and return a :class:`StepResult` so
the runner can record telemetry.
"""
        ...
        with p.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                event = blob.get("event") or blob.get("type")
                if event == "vault_health":
                    latest = blob
    except OSError:
        return None
    return latest
...
def read_wake_state(
    *,
    ...
    No LLM calls. ``wake_file_path`` is the wake.md driver file; if
    given, we parse its frontmatter for ``mode`` / ``time``. If absent
    or unreadable, we default to ``mode="sleep_b"`` and ``now`` for
    the time.
    """
...
def apply_action(
    ...
    *,
    ...
    """Apply one classified action to the filesystem.

    Returns a :class:`SurfacePayload` when the action wants something
    surfaced (RouteToSurface) — Step 6 collects these and writes them
    in one pass. Other actions write directly to their destination.
    """
    ...
    if isinstance(action, Discard):
        # Nothing to write — consume_note still moves the source.
        return None
...
def consume_note(note_path: pathlib.Path, *, today: _dt.date) -> pathlib.Path:
    """Move ``note_path`` to ``inner/notes/.consumed/<today>/``. Returns
    the new path.
    """
    ...
    if target.exists():
        target = (
            consumed_root / f"{note_path.stem}-{int(time.time())}{note_path.suffix}"
        )
...
async def drain_inbox(
    ...
    *,
    ...
    Per-note errors append to ``per_note_errors`` and the loop
    continues — one bad note doesn't drop the rest. ``apply_writes=False``
    is the shadow-mode lever: classify but don't write or consume.
    """
    actions: list[Action] = []
    ...
    for note_path in list(state.inbox_files):
        ...
        if not apply_writes:
            # Shadow mode — record what we'd do, don't touch disk.
            continue
        try:
...
def pick_grooming_target(state: WakeState) -> Optional[pathlib.Path]:
    """Step 3 — return the highest-scoring grooming candidate, or
    ``None`` if the vault has nothing worth grooming.
    """
...
def _split_frontmatter(text: str) -> tuple[dict[str, str], str]:
    """Return (frontmatter dict, body). If no frontmatter, dict is empty
...
def _replace_section(body: str, heading: str, new_section_body: str) -> str:
    """Replace the body under the named ATX heading.

    Heading match is case-insensitive on the heading TEXT (the part
    after ``#``s and surrounding whitespace). The replacement
    preserves the original heading line.
    """
    ...
    while i < len(lines):
        ...
        if m and m.group(2).strip().lower() == target and not replaced:
            ...
            # Skip until the next heading at <= level (or EOF).
            j = i + 1
            while j < len(lines):
                ...
                if nxt and len(nxt.group(1)) <= level:
                    break
                j += 1
            # Insert the new body. Ensure trailing newline so the
            # next heading starts on its own line.
...
def apply_diff(target_path: pathlib.Path, diff: Diff) -> bool:
    """Apply a typed :class:`Diff` to ``target_path``. Returns ``True``
    when the file content changed, ``False`` otherwise.
    """
    if diff.is_empty():
        return False
    ...
    # 2. Section edits — applied first so they don't clobber subsequent
    #    wikilink fixes within the replaced body.
    for edit in diff.section_edits:
        body = _replace_section(body, edit.heading, edit.new_body)

    # 3. Wikilink fixes — global string replacements. Run last so they
    #    catch any links inside freshly-replaced section bodies too.
...
async def groom_target(
    ...
    *,
    ...
    """Step 4 — produce + apply a grooming diff for ``target``.

    Returns ``None`` if no target, no changes, or the model call
    failed. Errors propagate to the caller, which records them on
    :class:`WakeState`.
    """
    if target is None:
        return None
...
async def stale_finding_lint(
    ...
    *,
    ...
    """One side-check branch — see if the target's stated problem is
    resolved by neighbor notes."""
    ...
    # Pull a couple of neighbor snippets from the same directory.
    neighbors: list[str] = []
    try:
        for sib in sorted(target.parent.glob("*.md")):
            if sib == target:
                continue
            try:
                snippet = sib.read_text(encoding="utf-8")
            except OSError:
                continue
            neighbors.append(snippet[:500])
            if len(neighbors) >= 3:
                break
    except OSError:
        pass
...
async def shadow_neighbor(
    ...
    *,
    ...
    """One side-check branch — bump a dormant neighbor of the target."""
    if target is None or not target.is_file():
        return SideCheckResult(
    ...
    # Find dormant neighbors — wikilinks whose access_count is 0.
    targets = set(m.group(1).strip() for m in _WIKILINK_RE.finditer(body))
    if not targets:
        return SideCheckResult(
    ...
    for slug in sorted(targets):
        # Cheap resolver — same-stem match anywhere under cortex-memory/.
    ...
    # Bump access_count via apply_diff.
    fm, _body = _split_frontmatter(dormant_body)
    try:
        ac = int(fm.get("access_count", "0") or 0)
    except ValueError:
        ac = 0
    ...
    # Use apply_diff for the section edit but write frontmatter directly
    # since apply_diff handles fm changes too.
    from .types import FrontmatterChange  # noqa: WPS433
    ...
    return SideCheckResult(
        name="shadow_neighbor",
        ok=True,
        action_summary=f"bumped {dormant.name}",
    )
...
async def conflict_scan(
    ...
    *,
    ...
    """One side-check branch — look for conflicts with recently-updated
    wikilinked neighbors."""
    if target is None or not target.is_file():
        return SideCheckResult(
            name="conflict_scan", ok=True, action_summary="no target"
        )
    try:
        body = target.read_text(encoding="utf-8")
    except OSError as exc:
    ...
    # Pull neighbor wikilinks (skip dailies/conflicts).
...
async def side_checks(
    ...
    *,
    ...
    """Step 5 — fan out the three side-check branches via
    :func:`asyncio.gather`. Each branch has a hard per-branch timeout;
    timeouts return ``None`` for that branch.
    """
    import asyncio
    ...
    sfl, sn, cs = await asyncio.gather(
        ...
        _wrap(
            "shadow_neighbor",
            shadow_neighbor(
                state, target, model_call=model_call, apply_writes=apply_writes
            ),
        ),
        _wrap(
            "conflict_scan",
    ...
    results = SideCheckResults(
        stale_finding_lint=sfl,
        shadow_neighbor=sn,
        conflict_scan=cs,
    )
    state.side_check_results = results
    # Surface any conflict-scan payloads up to wake state for Step 6.
...
def emit_surfaces(state: WakeState, *, apply_writes: bool = True) -> int:
    """Step 6 — write all collected :class:`SurfacePayload`s to
    ``inner/surface/``. Returns the count written.
    """
    ...
    # Reuse design_pipeline.write_surface so the on-disk shape stays
    # identical to other thinking-side surfaces.
...
def _build_summary_markdown(state: WakeState, results: list[StepResult]) -> str:
    ...
    if state.side_check_results is not None:
        for r in state.side_check_results.all():
            status = "ok" if r.ok else f"FAIL ({r.error})"
            lines.append(f"- {r.name}: {status} — {r.action_summary or ''}")
    else:
...
def _prune_rolling(mind_dir: pathlib.Path, *, today: _dt.date) -> None:
    """Step 5 housekeeping per CLAUDE.md — rolling deletes of
    ``inner/thoughts/``, ``inner/surface/.handled/``, and
    ``inner/notes/.consumed/`` entries older than the threshold.
    """
    targets = [
...
def close(
    ...
    *,
    ...
    """Step 7 — write the structured wake summary + run prune."""
    summary_path: Optional[pathlib.Path] = None
    if apply_writes:
        thoughts_dir = (
            state.mind_dir / "inner" / "thoughts" / state.now.date().isoformat()
        )
        try:
