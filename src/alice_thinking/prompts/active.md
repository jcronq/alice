...
If `sleep_window_closed` is false (the 23:00→07:00 window has not yet closed at the time of this wake), DO NOT set `stage_d_drought` — partial-window data is not sufficient to detect drought. Only check for drought when `sleep_window_closed` is true. If `stage_d == 0` for 3+ consecutive days while `research_notes_last_night > 0` exists, also append `stage_d_drought: true` to the event — Stage D is silently skipping despite eligible vault state. See [[2026-04-27-shadow-path-blindness]] for the precedent (84 Stage-B-only wakes ran unnoticed before Stage C/D were discovered missing).
