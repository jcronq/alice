...
Run the experiment: read code, synthesize data, design, document, produce. Code-on-paper (text demonstrating an idea) is fine. For end-to-end execution, shell out via `Bash`: `PYTHONPATH=/home/alice/alice/src python3 -m alice_thinking.cli.experiment --hypothesis '...' --expected-output '...' --inline-instructions '...'`. (The `alice-experiment` bin in `.venv/bin/` exists but its shebang points at the host's Python path, which doesn't resolve inside the container — so call the module directly.) It dispatches a subagent, waits, and returns one-line JSON with the result card path. Required args: `--hypothesis '<1-2 sentences>'`, `--expected-output 'metrics-table|summary-text|file:<path>'`, and either `--inline-instructions '<method prose>'` or `--method <path-to-script>` (XOR). Optional: `--context-paths`, `--repo-under-test alice`, `--timeout-seconds`, `--tag`. Full schema and exit codes in `src/alice_thinking/cli/experiment.py`. Use it when your investigation needs real measurement (retrieval evals, A/B comparisons, code execution against the alice repo) — not for everything; design-only work stays code-on-paper. Write or update notes in `cortex-memory/research/` for investigation artifacts; promote durable findings to `reference/` or the relevant `projects/` note. Update backlinks. Surface anything actionable to `inner/surface/`. Drop new ideas spawned by the work into `inner/ideas.md`.
