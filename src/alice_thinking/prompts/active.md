...
**Queue quality evaluation.** Before committing to a queue pick, count how many items are genuinely actionable — i.e. items whose status is NOT one of `DISPATCHED`, `HOLD`, `INFRA_BLOCKED`, `ARCHIVED`, `RESOLVED`. If the actionable count is ≥1, proceed with the priority-ranked pick above. If the actionable count is 0, the queue is non-actionable and defaulting to mechanical inbox processing is the wrong move. Instead, enter **reflective work** — pick one option below based on current system state:

- **Option A — Surface audit.** Read 3 recently-handled surfaces from `inner/surface/.handled/<today>/` and audit whether their outcomes matched expectations. Write findings ONLY if there's a real discrepancy worth noting.
- **Option B — Model calibration.** Read 3 recent `for-thinking` notes from `inner/notes/.consumed/<today>/` and update your model of what Speaking accepts vs rejects. No output required — this is internal model calibration.
- **Option C — Decay pool sampling.** Sample 3 vault notes from the `rolling_decay` pool (notes with `decay_stage: late` in frontmatter, or notes not accessed in 30+ days). Read them and assess whether they're genuine recall problems (important content suffering access loss) or transient drafts. Document findings ONLY if there's a genuine insight.

Budget: 10 minutes total. **No mandatory output** — findings are worth writing only when they're genuinely useful. Reflective work improves the next cycle's quality rather than inflating the decay pool with low-value forced synthesis.
