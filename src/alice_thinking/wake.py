...
def _stage_b_workflow_flags(mind: pathlib.Path) -> tuple[bool, bool]:
    ...
    Returns ``(workflow_enabled, shadow_enabled)``. Both default to
    ``False``. The cutover protocol per the design doc is:
    ...
    if not isinstance(think, dict):
        return False, False
    workflow = bool(think.get("stage_b_workflow_enabled", False))
    shadow = bool(think.get("stage_b_shadow_enabled", False))
    return workflow, shadow
...
def main() -> int:
    ...
    # Stage B workflow cutover — see
    # ``docs/designs/stage-b-cutover.md``. When the flag is true and
    # the selected phase is SLEEP_B, route through the typed-step
    # workflow instead of the prompt-driven SleepMode. Shadow flag is
    # honored separately: it runs the workflow alongside the prompt
    # path with apply_writes=False, so the cutover comparison can
    ...
    # path is still authoritative (workflow_enabled=false OR phase!=SLEEP_B),
    # also run the workflow with apply_writes=False so we can compare
    # outputs against the live writes. Shadow telemetry is tagged
    # ``stage_b_shadow_*`` so the viewer can filter it from real
    # ``stage_b_*`` events.
    if (
        not args.quick
        and shadow_enabled
