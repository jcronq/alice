...
from sm.dispatcher.constants import STUDY_HINT_WRITTEN_PREFIX
from sm.dispatcher.errors import GHCommandError
from sm.dispatcher.trust import _label_names
from sm.dispatcher.types import ListCommentsFn
...
def _find_parsed_comment_of_type(
    ...
    *,
    ...
    (:mod:`sm.comments`), so an untrusted commenter pasting one
    of the canonical verbs parses to ``None`` and is silently skipped.
    """
    from sm.comments import parse_comment
...
def _research_close_signal(
    ...
    We import :mod:`sm.comments` lazily to avoid the import cycle
    (comments imports ``TRUSTED_AUTHORS`` / ``ART_LABEL_WHITELIST`` from
    this module at top level).
    """
    from sm import comments as cm  # local import — avoid cycle
