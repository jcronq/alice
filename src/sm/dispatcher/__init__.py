...
right now it runs by hand via ``python -m sm.dispatcher``. The
...
(``from sm.dispatcher import X``) is unchanged for every callsite
that existed pre-split:

* :mod:`sm.dispatcher.constants`  — labels, prefixes, paths,
  caps, the SPAWN_MAP table, the ``_now_iso`` helper.
* :mod:`sm.dispatcher.errors`     — :class:`GHCommandError`.
* :mod:`sm.dispatcher.types`      — ``Callable`` type aliases.
* :mod:`sm.dispatcher.state`      — :class:`DispatcherState`,
  load/save round-trip for the dedup ledger.
* :mod:`sm.dispatcher.report`     — :class:`RunReport`,
  :class:`DependencyResolution`, :func:`resolve_dependencies`.
* :mod:`sm.dispatcher.trust`      — :class:`TrustDecision`,
  :func:`evaluate_trust`, label-extraction helpers.
* :mod:`sm.dispatcher.gh`         — every ``gh`` CLI shim
  (``gh_list_*``, ``gh_post_comment``, ``gh_edit_labels``, …).
* :mod:`sm.dispatcher.git_ops`    — ``_run_git``,
  ``_post_merge_cleanup``, ``_attempt_auto_rebase``.
* :mod:`sm.dispatcher.spawn`      — three spawn lanes (worker,
  thinking, speaking) plus rebase-spawn and shared liveness machinery.
* :mod:`sm.dispatcher.rendering`  — ``render_*`` audit-comment
  renderers (incl. ``REBASE_*_PREFIX`` constants).
* :mod:`sm.dispatcher.verify`     — issue #128 viewer-route
  smoke-test gate.
* :mod:`sm.dispatcher.helpers`    — shared internal helpers
  (``_comment_author_login``, ``_find_parsed_comment_of_type`` …).
* :mod:`sm.dispatcher.handlers`   — one file per state-handler
  ...
  :mod:`sm.dispatcher.handlers._common`.
* :mod:`sm.dispatcher.main`       — :func:`run` (one pass) and
...
# ---------------------------------------------------------------------------
# Constants — re-exported from sm.dispatcher.constants.
#
# If ``constants`` is already loaded when this package is re-executed
# (``importlib.reload(sm.dispatcher)``), reload it first so the
...
from sm.dispatcher.types import (  # noqa: E402, F401
    CloseIssueFn,
    EditLabelsFn,
    FindLinkedPRFn,
    FindUnspawnedFn,
...
# State persistence
from sm.dispatcher.state import (  # noqa: E402, F401
    DispatcherState,
    load_state,
    save_state,
)
...
from sm.dispatcher.report import (  # noqa: E402, F401
...
# Trust filter
from sm.dispatcher.trust import (  # noqa: E402, F401
    TrustDecision,
    _author_login,
    _current_sm_label,
    _label_names,
...
# gh CLI shims
from sm.dispatcher.gh import (  # noqa: E402, F401
    _run_gh,
    _sort_oldest_first,
    gh_close_issue,
    gh_edit_labels,
...
# Git operations
from sm.dispatcher.git_ops import (  # noqa: E402, F401
...
# Spawn machinery
from sm.dispatcher.spawn import (  # noqa: E402, F401
    _SPAWN_DIR_NAME_RE,
...
# Comment rendering
from sm.dispatcher.rendering import (  # noqa: E402, F401
...
from sm.dispatcher.verify import (  # noqa: E402, F401
    _http_get_body,
    _touches_viewer,
    _verify_enabled,
    default_verifier,
...
from sm.dispatcher.helpers import (  # noqa: E402, F401
...
from sm.dispatcher.handlers.building import _process_building  # noqa: E402, F401
from sm.dispatcher.handlers.compacting import _process_compacting  # noqa: E402, F401
from sm.dispatcher.handlers.design_review import _process_design_review  # noqa: E402, F401
from sm.dispatcher.handlers.designed import (  # noqa: E402, F401
    _designed_spawn_speaking,
    _process_designed,
)
from sm.dispatcher.handlers.designing import _process_designing  # noqa: E402, F401
from sm.dispatcher.handlers.draft import _process_draft  # noqa: E402, F401
from sm.dispatcher.handlers.needs_study import _process_needs_study  # noqa: E402, F401
from sm.dispatcher.handlers.open_done import _process_open_done  # noqa: E402, F401
from sm.dispatcher.handlers.reviewing import (  # noqa: E402, F401
    _handle_conflicting_pr,
    _process_reviewing,
)
from sm.dispatcher.handlers.selected import _process_selected  # noqa: E402, F401
from sm.dispatcher.handlers.stale_closed import _process_stale_closed  # noqa: E402, F401

# Dispatcher main pass + CLI. Re-exporting ``main`` here is what makes
# the ``alice-sm = "sm.dispatcher:main"`` console-script entry in
# ``pyproject.toml`` resolve, and what keeps ``sm.dispatcher.run(...)``
# importable for downstream callers.
from sm.dispatcher.main import main, run  # noqa: E402, F401
