...
The :data:`SPAWN_MAP` table in :mod:`sm.dispatcher.constants`
...
from sm.dispatcher.gh import gh_post_comment
from sm.dispatcher.trust import _author_login, _current_sm_label, _label_names
from sm.dispatcher.types import PostCommentFn
...
def _current_spawn_map() -> dict[tuple[str, str], dict[str, str]]:
    ...
    ``sys.modules['sm.dispatcher']``, restoring the contract.
    """
    return sys.modules["sm.dispatcher"].SPAWN_MAP
...
def spawn_thinking_agent(
    ...
    *,
    ...
    entrypoint replaces :mod:`sm.thinking_shim` in sub-issue 3.
    """
    if clock is None:
        clock = time.time
    ...
    *,
    ...
    Steps (mirror of :func:`spawn_thinking_agent`):
      ...
      5. Launch the speaking shim detached via
         ``python -m sm.speaking_shim --spawn-dir <dir> --session-id <uuid> --mode build``
         with stdout/stderr to log files, ``start_new_session=True`` so
         the agent survives the dispatcher exiting.
      6. Write PID to ``pidfile``.
