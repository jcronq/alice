...
from alice_forge.dispatcher.constants import STUDY_HINT_WRITTEN_PREFIX
from alice_forge.dispatcher.errors import GHCommandError
from alice_forge.dispatcher.trust import _label_names
from alice_forge.dispatcher.types import ListCommentsFn
...
def _find_parsed_comment_of_type(
    ...
    *,
    ...
    (:mod:`alice_forge.comments`), so an untrusted commenter pasting one
    of the canonical verbs parses to ``None`` and is silently skipped.
    """
    from alice_forge.comments import parse_comment
...
def _research_close_signal(
    ...
    We import :mod:`alice_forge.comments` lazily to avoid the import cycle
    (comments imports ``TRUSTED_AUTHORS`` / ``ART_LABEL_WHITELIST`` from
    this module at top level).
    """
    from alice_forge import comments as cm  # local import — avoid cycle
