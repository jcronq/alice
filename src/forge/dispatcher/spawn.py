...
The :data:`SPAWN_MAP` table in :mod:`forge.dispatcher.constants`
...
from forge.dispatcher.gh import gh_post_comment
from forge.dispatcher.trust import _author_login, _current_sm_label, _label_names
from forge.dispatcher.types import PostCommentFn
...
def _current_spawn_map() -> dict[tuple[str, str], dict[str, str]]:
    ...
    ``sys.modules['forge.dispatcher']``, restoring the contract.
    """
    return sys.modules["forge.dispatcher"].SPAWN_MAP
    ...
    *,
    ...
    Steps:
      ...
      5. Launch the thinking shim detached via
         ``python -m forge.thinking_shim --spawn-dir <dir> --session-id <uuid>``
    ...
    entrypoint replaces :mod:`forge.thinking_shim` in sub-issue 3.
    """
    if clock is None:
        clock = time.time
    ...
    *,
    ...
    Steps (mirror of :func:`spawn_thinking_agent`):
      ...
      5. Launch the speaking shim detached via
         ``python -m forge.speaking_shim --spawn-dir <dir> --session-id <uuid> --mode build``
         with stdout/stderr to log files, ``start_new_session=True`` so
         the agent survives the dispatcher exiting.
      6. Write PID to ``pidfile``.
