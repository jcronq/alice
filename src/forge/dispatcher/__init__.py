...
Modeled on :mod:`watchers.github`. Each invocation is a single pass:
...
right now it runs by hand via ``python -m forge.dispatcher``. The
...
(``from forge.dispatcher import X``) is unchanged for every callsite
that existed pre-split:

* :mod:`forge.dispatcher.constants`  — labels, prefixes, paths,
  caps, the SPAWN_MAP table, the ``_now_iso`` helper.
* :mod:`forge.dispatcher.errors`     — :class:`GHCommandError`.
* :mod:`forge.dispatcher.types`      — ``Callable`` type aliases.
* :mod:`forge.dispatcher.state`      — :class:`DispatcherState`,
  load/save round-trip for the dedup ledger.
* :mod:`forge.dispatcher.report`     — :class:`RunReport`,
  :class:`DependencyResolution`, :func:`resolve_dependencies`.
* :mod:`forge.dispatcher.trust`      — :class:`TrustDecision`,
  :func:`evaluate_trust`, label-extraction helpers.
* :mod:`forge.dispatcher.gh`         — every ``gh`` CLI shim
  (``gh_list_*``, ``gh_post_comment``, ``gh_edit_labels``, …).
* :mod:`forge.dispatcher.git_ops`    — ``_run_git``,
  ``_post_merge_cleanup``, ``_attempt_auto_rebase``.
* :mod:`forge.dispatcher.spawn`      — three spawn lanes (worker,
  thinking, speaking) plus rebase-spawn and shared liveness machinery.
* :mod:`forge.dispatcher.rendering`  — ``render_*`` audit-comment
  renderers (incl. ``REBASE_*_PREFIX`` constants).
* :mod:`forge.dispatcher.verify`     — issue #128 viewer-route
  smoke-test gate.
* :mod:`forge.dispatcher.helpers`    — shared internal helpers
  (``_comment_author_login``, ``_find_parsed_comment_of_type`` …).
* :mod:`forge.dispatcher.handlers`   — one file per state-handler
  ...
  :mod:`forge.dispatcher.handlers._common`.
* :mod:`forge.dispatcher.main`       — :func:`run` (one pass) and
...
# ---------------------------------------------------------------------------
# Constants — re-exported from forge.dispatcher.constants.
#
# If ``constants`` is already loaded when this package is re-executed
# (``importlib.reload(forge.dispatcher)``), reload it first so the
...
from forge.dispatcher.constants import *  # noqa: F401, F403, E402
from forge.dispatcher.constants import _now_iso  # noqa: F401, E402
from forge.dispatcher.constants import (  # noqa: F401, E402
...
# Errors
from forge.dispatcher.errors import GHCommandError  # noqa: E402, F401

# Callable type aliases
from forge.dispatcher.types import (  # noqa: E402, F401
    CloseIssueFn,
    EditLabelsFn,
    FindLinkedPRFn,
    FindUnspawnedFn,
...
# State persistence
from forge.dispatcher.state import (  # noqa: E402, F401
    DispatcherState,
    load_state,
    save_state,
)
...
from forge.dispatcher.report import (  # noqa: E402, F401
...
# Trust filter
from forge.dispatcher.trust import (  # noqa: E402, F401
    TrustDecision,
    _author_login,
    _current_sm_label,
    _label_names,
...
# gh CLI shims
from forge.dispatcher.gh import (  # noqa: E402, F401
    _run_gh,
    _sort_oldest_first,
    gh_close_issue,
    gh_edit_labels,
...
# Git operations
from forge.dispatcher.git_ops import (  # noqa: E402, F401
...
# Spawn machinery
from forge.dispatcher.spawn import (  # noqa: E402, F401
    _SPAWN_DIR_NAME_RE,
...
# Comment rendering
from forge.dispatcher.rendering import (  # noqa: E402, F401
...
from forge.dispatcher.verify import (  # noqa: E402, F401
    _http_get_body,
    _touches_viewer,
    _verify_enabled,
    default_verifier,
...
from forge.dispatcher.helpers import (  # noqa: E402, F401
...
from forge.dispatcher.handlers.building import _process_building  # noqa: E402, F401
from forge.dispatcher.handlers.compacting import _process_compacting  # noqa: E402, F401
from forge.dispatcher.handlers.design_review import _process_design_review  # noqa: E402, F401
from forge.dispatcher.handlers.designed import (  # noqa: E402, F401
    _designed_spawn_speaking,
    _process_designed,
)
from forge.dispatcher.handlers.designing import _process_designing  # noqa: E402, F401
from forge.dispatcher.handlers.draft import _process_draft  # noqa: E402, F401
from forge.dispatcher.handlers.needs_study import _process_needs_study  # noqa: E402, F401
from forge.dispatcher.handlers.open_done import _process_open_done  # noqa: E402, F401
from forge.dispatcher.handlers.reviewing import (  # noqa: E402, F401
    _handle_conflicting_pr,
    _process_reviewing,
)
from forge.dispatcher.handlers.selected import _process_selected  # noqa: E402, F401
from forge.dispatcher.handlers.stale_closed import _process_stale_closed  # noqa: E402, F401

# Dispatcher main pass + CLI. Re-exporting ``main`` here is what makes
# the ``alice-sm = "forge.dispatcher:main"`` console-script entry in
# ``pyproject.toml`` resolve, and what keeps ``forge.dispatcher.run(...)``
# importable for downstream callers.
from forge.dispatcher.main import main, run  # noqa: E402, F401
