...
from forge.dispatcher.constants import STUDY_HINT_WRITTEN_PREFIX
from forge.dispatcher.errors import GHCommandError
from forge.dispatcher.trust import _label_names
from forge.dispatcher.types import ListCommentsFn
...
def _find_resolving_research_note(
    ...
    if not research_dir.is_dir():
        return None
    # Local import — :mod:`indexer.yaml_lite` lives in a sibling
...
def _find_parsed_comment_of_type(
    ...
    *,
    ...
    (:mod:`forge.comments`), so an untrusted commenter pasting one
    of the canonical verbs parses to ``None`` and is silently skipped.
    """
    from forge.comments import parse_comment
...
def _research_close_signal(
    ...
    We import :mod:`forge.comments` lazily to avoid the import cycle
    (comments imports ``TRUSTED_AUTHORS`` / ``ART_LABEL_WHITELIST`` from
    this module at top level).
    """
    from forge import comments as cm  # local import — avoid cycle
