...
:func:`alice_sm.dispatcher.spawn_speaking_agent` for each
...
Thin wrapper around :mod:`alice_sm.speaking_shim` — the real
PhaseRunner-based logic lands in the follow-up sub-issue that replaces
the placeholder shim. Both entry forms (``python -m alice_sm.speaking_shim``
...
def _ensure_src_on_path() -> None:
    ...
    so ``import alice_sm`` resolves to the in-tree package. The venv
...
def main() -> int:
    _ensure_src_on_path()
    from alice_sm.speaking_shim import main as _main
