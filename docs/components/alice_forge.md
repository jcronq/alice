...
**TBD** — the idea state machine. Tracks ideas through `draft → selected → designing → designed → building → reviewing → validating → done` (with `blocked` and `rejected` escapes). Source: `src/alice_forge/`.
