...
Other packages (`alice_core`, `alice_daemon`, `alice_eval`, `alice_indexer`, `alice_metrics`, `alice_prompts`, `alice_skills`, `alice_viewer`) don't have stubs yet — add one here the first time you write a PR that needs to point at one. The pi-coding-agent subprocess kernel lives under `alice_core/kernel/pi/`.
