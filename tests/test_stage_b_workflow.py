"""Tests for ``alice_thinking.workflows.stage_b``.

Per-step + integration coverage. Mocks the LLM at the
:class:`ModelCall` seam — no real model calls in CI.

Pin: action routing, deterministic scoring, Diff application,
parallel side-checks + per-branch timeouts, surface emission shape,
end-to-end full-workflow integration with telemetry assertions.
"""
...
class ScriptedModel:
    """Deterministic ModelCall stand-in. Returns canned responses by
    matching against the system_prompt fragment header.

    Each entry is keyed by the prompt fragment NAME (e.g.
    ``classify_note``) and is either a string or a callable
    ``(system_prompt, user_prompt) -> str``. Lists rotate through.
    """
    ...
    async def __call__(self, system_prompt: str, user_prompt: str) -> str:
        ...
        for key, val in self._responses.items():
            if f"## {key}" in system_prompt:
                return self._resolve(key, val, system_prompt, user_prompt)
        raise AssertionError(
            f"ScriptedModel: no response for system_prompt starting with "
            f"{system_prompt[:80]!r}"
        )

    def _resolve(self, key: str, val: Any, system: str, user: str) -> str:
...
def test_read_wake_state_parses_typed_state(tmp_path: pathlib.Path) -> None:
    ...
    # Two inbox notes, one hidden, one already-consumed.
    ...
    # vault_health event in events.jsonl.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _action_response("append_to_daily", line="ate breakfast"),
            AppendToDaily,
        ),
        (
            _action_response(
                "create_conflict_note",
                slug="weight-mismatch",
                body="conflict body",
            ),
            CreateConflictNote,
        ),
...
def test_drain_inbox_consumes_processed_notes(tmp_path: pathlib.Path) -> None:
    ...
    assert not note.exists()  # moved to .consumed
...
def test_drain_inbox_continues_on_per_note_error(tmp_path: pathlib.Path) -> None:
    ...
    # First call raises (model fail), second returns a clean discard.
    responses = [
        "not valid json",
        _action_response("discard", reason="noise"),
    ]
    model = ScriptedModel({"classify_and_route_note": responses})
    result = asyncio.run(steps_mod.drain_inbox(state, model_call=model))

    assert len(result.actions) == 1  # only the second one succeeded
    assert len(result.per_note_errors) == 1
    assert "001-bad.md" in result.per_note_errors[0]
    # Good note got consumed, bad note stayed put.
...
def test_pick_grooming_target_deterministic_scoring(tmp_path: pathlib.Path) -> None:
    ...
    # Stale + low_access — score 2.
    (research / "stale-low.md").write_text(
        "---\nupdated: 2025-01-01\naccess_count: 0\n---\n\nbody\n",
        encoding="utf-8",
    )
    # Fresh + high access — score 0 → not returned.
    ...
    # Lex tiebreak: two notes with score 2.
    ...
    # Lexicographic tiebreak is on str(path), so the path with
    # "another-stale-low" sorts before "stale-low".
...
def test_groom_target_applies_diff(tmp_path: pathlib.Path) -> None:
    ...
    # Two sections: links in "Body", an "Aside" we'll rewrite.
    target.write_text(
        "---\nupdated: 2025-01-01\naccess_count: 0\ntitle: old\n---\n\n"
    ...
    model = ScriptedModel(
        {
            "produce_grooming_diff": _diff_response(
                ...
                wikilink_fixes=[
                    {"old_target": "bad-link", "new_target": "good-link"},
                ],
                section_edits=[
    ...
    assert "[[other-link]]" in after  # untouched
...
def test_apply_diff_section_edit_replaces_named_heading(tmp_path: pathlib.Path) -> None:
    ...
    diff = Diff(
        section_edits=[SectionEdit(heading="A", new_body="new aaa")],
    )
    assert steps_mod.apply_diff(target, diff) is True
    after = target.read_text()
    assert "new aaa" in after
    assert "bbb" in after  # other section preserved
...
def test_side_checks_run_in_parallel(tmp_path: pathlib.Path) -> None:
    ...
    # Sequential calls would take ~0.6s; parallel should be ~0.2s.
    ...
    # All three branches succeeded; total elapsed under ~0.5s shows parallelism.
    assert all(r is not None and r.ok for r in results.all())
    assert elapsed < 0.5
    # The shadow neighbor's access_count got bumped.
...
def test_side_check_timeout_returns_none(tmp_path: pathlib.Path) -> None:
    ...
    # Each branch should report ok=False with error="timeout".
...
def test_close_writes_summary_and_runs_prune(tmp_path: pathlib.Path) -> None:
    paths = _build_mind(tmp_path)
    mind = paths["mind"]
    # Plant an old thoughts dir + a young one.
    ...
    # Old thoughts dir got pruned (>7 days older than today=2026-05-08).
    assert not old_dir.exists()
    # Young thoughts dir survived.
...
def test_wake_closes_cleanly_on_step_error(tmp_path: pathlib.Path) -> None:
    """A step that raises should NOT take down the wake. Step 7
    runs and emits a summary even when an earlier step failed.
    """
    paths = _build_mind(tmp_path)
    ...
    # Wake closed (Step 7 ran), even though earlier steps had no model.
    assert summary.summary_path is not None
    assert summary.summary_path.is_file()
    # Drain inbox failed per-note (caught inside the step), so steps_failed=0
    # at the runner-step level — but per_note_errors was non-empty.
...
def test_full_workflow_end_to_end_with_mocks(tmp_path: pathlib.Path) -> None:
    """Synthetic vault + scripted ModelCall — runs the full graph, asserts
    side-effects + telemetry."""
    paths = _build_mind(tmp_path)
    mind = paths["mind"]
    # Two inbox notes — promote + append-to-daily.
    ...
    # Plant a stale + low-access note for grooming.
    ...
    # Plant a neighbor for side-checks.
    ...
    side_responses = {
        "stale_finding_lint": json.dumps({"verdict": "still_open", "summary": "ok"}),
    ...
    # 1. Inbox drained — both notes consumed, both actions applied.
    ...
    # 2. Grooming target picked + groomed.
    after = target.read_text()
    assert "updated: 2026-05-08" in after
    assert "[[fixed-link]]" in after
    # 3. Wake summary written under inner/thoughts/<today>/.
    assert summary.summary_path is not None and summary.summary_path.is_file()
    # 4. Telemetry events emitted: one per step + a wake_summary.
...
def test_shadow_mode_does_not_write_or_consume(tmp_path: pathlib.Path) -> None:
    ...
    # Note still in inbox — not consumed.
    assert note.is_file()
    # Target not modified.
