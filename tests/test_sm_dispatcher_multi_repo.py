...
Covers :func:`sm.dispatcher.load_dispatcher_repos` parsing across
the three documented config shapes (missing, malformed, present) and
the relaxed-label mode in :func:`sm.dispatcher.run` for repos
...
def test_load_dispatcher_repos_parses_full_config(tmp_path: pathlib.Path) -> None:
    ...
    # ``sm.dispatcher.__init__``'s reload-on-reimport of constants.
    assert len(repos) == 2
