...
``core/`` is the kernel: it depends on Python stdlib, the
...
The check: walk every ``.py`` under ``src/core/``, AST-parse
...
CORE_DIR = pathlib.Path(__file__).resolve().parents[1] / "src" / "core"
...
def _toplevel_imports(tree: ast.AST) -> list[str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if node.level > 0:
                # Relative import — stays inside core. Skip.
                continue
...
def test_core_imports_only_sdk_stdlib_or_self(py_file: pathlib.Path):
    """Every import in ``core/`` must resolve to stdlib, the
    ...
    ``sm``, ``metrics``, ``eval``) are forbidden — the
    ...
    for name in _toplevel_imports(tree):
        assert name in ALLOWED_TOPLEVEL, (
            ...
            f"{name!r} — core must depend only on stdlib, "
...
def test_allow_list_contains_expected_anchors():
    ...
    assert "core" in ALLOWED_TOPLEVEL
