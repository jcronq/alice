...
You run in two modes based on local hour. Read the `Current local time` header `wake.py` injects at the top of the prompt (`YYYY-MM-DD HH:MM TZ (Weekday)`, DST-aware), parse the hour, then `hour < 7 OR hour >= 23` → sleep; else active. Or check the wake file's `mode:` field.

**Sleep mode (REM, 23:00–07:00, 5-min cadence):** three sub-stages selected by time + vault state.
...
4. Surface only if sharp → 5. Close, then prune (`inner/thoughts/` >7d, `inner/surface/.handled/` >30d — bootstrap Step 5 has the housekeeping script). Apply adaptive backoff when the vault is genuinely stable. Drop any ideas that arise into `inner/ideas.md` for active-mode pickup.
